...
class PrimerAutoCompleteItemTest < Minitest::Test
  ...
  def test_renders_item_with_correct_attributes
    render_inline(Primer::AutoComplete::Item.new(value: "foo")) { "Item" }

    assert_selector("li[role=\"option\"][data-autocomplete-value=\"foo\"].autocomplete-item", text: "Item")
  end

  def test_renders_selected_item
    render_inline(Primer::AutoComplete::Item.new(value: "foo", selected: true)) { "Item" }
  ...
  def test_renders_disabled_item
    render_inline(Primer::AutoComplete::Item.new(value: "foo", disabled: true)) { "Item" }
