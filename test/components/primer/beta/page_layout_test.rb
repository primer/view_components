...
class PrimerBetaPageLayoutTest < Minitest::Test
  ...
  def test_wrapper_sizing
    Primer::Beta::BaseLayout::WRAPPER_SIZING_OPTIONS.each do |size|
  ...
  def test_outer_spacing
    Primer::Beta::BaseLayout::OUTER_SPACING_OPTIONS.each do |size|
      render_inline(Primer::Beta::PageLayout.new(outer_spacing: size)) do |c|
        c.main { "Main" }
        c.pane { "Pane" }
      end

      size_class = Primer::Beta::BaseLayout::OUTER_SPACING_MAPPINGS[size]
  ...
  def test_column_gap
    Primer::Beta::BaseLayout::COLUMN_GAP_OPTIONS.each do |size|
      render_inline(Primer::Beta::PageLayout.new(column_gap: size)) do |c|
        c.main { "Main" }
        c.pane { "Pane" }
      end

      size_class = Primer::Beta::BaseLayout::COLUMN_GAP_MAPPINGS[size]
      assert_selector("div.LayoutBeta#{size_class.empty? ? '' : ".#{size_class}"}") do
        assert_selector("div.LayoutBeta-content", text: "Main")
        assert_selector("div.LayoutBeta-pane", text: "Pane")
      end
  ...
  def test_row_gap
    Primer::Beta::BaseLayout::ROW_GAP_OPTIONS.each do |size|
      render_inline(Primer::Beta::PageLayout.new(row_gap: size)) do |c|
        c.main { "Main" }
        c.pane { "Pane" }
      end

      size_class = Primer::Beta::BaseLayout::ROW_GAP_MAPPINGS[size]
      assert_selector("div.LayoutBeta#{size_class.empty? ? '' : ".#{size_class}"}") do
        assert_selector("div.LayoutBeta-content", text: "Main")
        assert_selector("div.LayoutBeta-pane", text: "Pane")
      end
  ...
  def test_responsive_primary_region
    Primer::Beta::BaseLayout::RESPONSIVE_PRIMARY_REGION_OPTIONS.each do |region|
      render_inline(Primer::Beta::PageLayout.new(responsive_primary_region: region)) do |c|
        c.main { "Main" }
        c.pane { "Pane" }
      end

      region_class = Primer::Beta::BaseLayout::RESPONSIVE_PRIMARY_REGION_MAPPINGS[region]
  ...
  def test_responsive_variant
    Primer::Beta::BaseLayout::RESPONSIVE_VARIANT_OPTIONS.each do |variant|
      render_inline(Primer::Beta::PageLayout.new(responsive_variant: variant)) do |c|
        c.main { "Main" }
        c.pane { "Pane" }
      end

      variant_class = Primer::Beta::BaseLayout::RESPONSIVE_VARIANT_MAPPINGS[variant]
  ...
  def test_pane_position_add_correct_class
    Primer::Beta::BaseLayout::Pane::POSITION_OPTIONS.each do |position|
      render_inline(Primer::Beta::PageLayout.new) do |c|
        c.main { "Main" }
  ...
  def test_pane_responsive_position
    Primer::Beta::BaseLayout::PANE_RESPONSIVE_POSITION_OPTIONS.each do |position|
      render_inline(Primer::Beta::PageLayout.new) do |c|
        c.main { "Main" }
        c.pane(responsive_position: position) { "Pane" }
      end

      position_class = Primer::Beta::BaseLayout::PANE_RESPONSIVE_POSITION_MAPPINGS[position]
  ...
  def test_pane_width
    Primer::Beta::BaseLayout::PANE_WIDTH_OPTIONS.each do |size|
      render_inline(Primer::Beta::PageLayout.new) do |c|
        c.main { "Main" }
        c.pane(width: size) { "Pane" }
      end

      width_class = Primer::Beta::BaseLayout::PANE_WIDTH_MAPPINGS[size]
      assert_selector("div.LayoutBeta") do
  ...
  def test_main_width
    Primer::Beta::BaseLayout::Main::WIDTH_OPTIONS.each do |width|
      render_inline(Primer::Beta::PageLayout.new) do |c|
  ...
  def test_pane_responsive_divider
    Primer::Beta::BaseLayout::Pane::RESPONSIVE_DIVIDER_OPTIONS.each do |type|
      render_inline(Primer::Beta::PageLayout.new) do |c|
        c.main { "Main" }
        c.pane(responsive_divider: type) { "Pane" }
      end

      type_class = Primer::Beta::BaseLayout::Pane::RESPONSIVE_DIVIDER_MAPPINGS[type]
      assert_selector("div.LayoutBeta") do
        assert_selector("div.LayoutBeta-content", text: "Main")
  ...
  def test_header_responsive_divider
    Primer::Beta::BaseLayout::Bookend::RESPONSIVE_DIVIDER_OPTIONS.each do |opt|
      render_inline(Primer::Beta::PageLayout.new) do |c|
      ...
      divider_class = Primer::Beta::BaseLayout::Bookend::RESPONSIVE_DIVIDER_MAPPINGS[opt]
      assert_selector("div.LayoutBeta") do
