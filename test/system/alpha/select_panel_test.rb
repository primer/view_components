...
module Alpha
  class IntegrationSelectPanelTest < System::TestCase
    ...
    def test_invoker_clicked_event_is_fired
      visit_preview(:default)

      # Add event listener to capture the invokerClicked event
      page.evaluate_script(<<~JS)
        window.selectPanelInvokerClicked = false
        document.querySelector('select-panel button[aria-controls]').addEventListener('invokerClicked', () => {
          window.selectPanelInvokerClicked = true
        })
      JS

      click_on_invoker_button

      assert page.evaluate_script("window.selectPanelInvokerClicked"), "invokerClicked event was not fired"
    end

    def test_invoker_clicked_event_is_fired_on_keyboard_enter
      visit_preview(:default)

      # Add event listener to capture the invokerClicked event
      page.evaluate_script(<<~JS)
        window.selectPanelInvokerClickedEnter = false
        document.querySelector('select-panel button[aria-controls]').addEventListener('invokerClicked', () => {
          window.selectPanelInvokerClickedEnter = true
        })
      JS
      ...
      assert page.evaluate_script("window.selectPanelInvokerClickedEnter"), "invokerClicked event was not fired on keyboard enter"
    end

    def test_invoker_clicked_event_is_fired_on_keyboard_space
      visit_preview(:default)

      # Add event listener to capture the invokerClicked event
      page.evaluate_script(<<~JS)
        window.selectPanelInvokerClickedSpace = false
        document.querySelector('select-panel button[aria-controls]').addEventListener('invokerClicked', () => {
          window.selectPanelInvokerClickedSpace = true
        })
      JS
      ...
      assert page.evaluate_script("window.selectPanelInvokerClickedSpace"), "invokerClicked event was not fired on keyboard space"
    end

    def test_invoker_clicked_event_bubbles
      visit_preview(:default)

      # Add event listener to parent element to test bubbling
      page.evaluate_script(<<~JS)
        window.selectPanelInvokerClickedBubbled = false
        document.querySelector('select-panel').addEventListener('invokerClicked', () => {
          window.selectPanelInvokerClickedBubbled = true
        })
      JS

      click_on_invoker_button

      assert page.evaluate_script("window.selectPanelInvokerClickedBubbled"), "invokerClicked event did not bubble to parent"
    end
