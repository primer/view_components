...
Kuby.define("ViewComponentsDemo") do
  shared = -> do
    docker do
      credentials do
      ...
      insert :main_yarn, before: :yarn_phase do |dockerfile|
        ...
        dockerfile.run("mkdir", "-p", "lookbook/app/assets/stylesheets")
      ...
      insert :build_storybook, after: :assets_phase do |dockerfile|
        ...
        dockerfile.run("yarn", "run", "build:css")
        dockerfile.run("yarn", "run", "build:ts")
        dockerfile.run("yarn", "run", "build:rollup")
      end
    end
