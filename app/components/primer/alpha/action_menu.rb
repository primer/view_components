  ...
  module Alpha
    ...
    class ActionMenu < Primer::Component
      ...
      renders_one :trigger, lambda { |icon: nil, with_caret: false, **system_arguments|
        if icon
          Primer::IconButton.new(role: "button", icon: icon, "aria-haspopup": true, "aria-expanded": false, "aria-controls": list_id, id: menu_id, **system_arguments)
        else
          Primer::ButtonComponent.new(role: "button", dropdown: with_caret, "aria-haspopup": true, "aria-controls": list_id, "aria-expanded": false, id: menu_id, **system_arguments)
        end
      }
      ...
      #      Primer Design
      #    <% end %>
      #    <% c.item(tag: :button, type: "button") do %>
      #      Quote Reply
      #    <% end %>
      #    <% c.item(tag: :"clipboard-copy", value: "Text to copy") do %>
      #      Copy Text
      ...
      #      Copy Text
      #    <% end %>
      #    <% c.item(tag: :button, type: "button", is_dangerous: true) do %>
      #      Delete
      #    <% end %>
      #  <% end %>
      #
