...
module Primer
  module Alpha
    ...
    class Layout < Primer::Component
      ...
      #
      # @param width [Symbol] <%= one_of(Primer::Experimental::Layout::Main::WIDTH_OPTIONS) %>
      # @param system_arguments [Hash] <%= link_to_system_arguments_docs %>
      renders_one :main, "Primer::Experimental::Layout::Main"

      # The layout's sidebar.
      #
      # @param width [Symbol] <%= one_of(Primer::Experimental::Layout::SIDEBAR_WIDTH_OPTIONS) %>
      # @param col_placement [Symbol] Sidebar placement when `Layout` is in column mode. <%= one_of(Primer::Experimental::Layout::SIDEBAR_COL_PLACEMENT_OPTIONS) %>
      # @param row_placement [Symbol] Sidebar placement when `Layout` is in row mode. <%= one_of(Primer::Experimental::Layout::SIDEBAR_ROW_PLACEMENT_OPTIONS) %>
      # @param system_arguments [Hash] <%= link_to_system_arguments_docs %>
      ...
      |
        ...
        Primer::Experimental::Layout::Sidebar.new(**system_arguments)
      }

      # @example Default
      #
      #   <%= render(Primer::Experimental::Layout.new) do |c| %>
      ...
      #       <% c.sidebar(border: true) { "Sidebar" } %>
      #     <% end %>
      #     <%= render(Primer::Experimental::Layout.new(mt: 5)) do |c| %>
      ...
      #
      #   @code
      #     <%= render(Primer::Experimental::Layout.new(stacking_breakpoint: :sm)) do |c| %>
      #       <% c.main(border: true) { "Main" } %>
      #       <% c.sidebar(border: true) { "Sidebar" } %>
      #     <% end %>
      #     <%= render(Primer::Experimental::Layout.new(stacking_breakpoint: :md, mt: 5)) do |c| %>
      #       <% c.main(border: true) { "Main" } %>
      #       <% c.sidebar(border: true) { "Sidebar" } %>
      #     <% end %>
      #     <%= render(Primer::Experimental::Layout.new(stacking_breakpoint: :lg, mt: 5)) do |c| %>
      #       <% c.main(border: true) { "Main" } %>
      #       <% c.sidebar(border: true) { "Sidebar" } %>
      #     <% end %>
      #
      # @param stacking_breakpoint [Symbol] When the `Layout` should change from rows into columns. <%= one_of(Primer::Experimental::Layout::STACKING_BREAKPOINT_OPTIONS) %>
      # @param first_in_source [Symbol] Which element to render first in the HTML. This will change the keyboard navigation order.
      # @param gutter [Symbol] The amount of space between the main section and the sidebar. <%= one_of(Primer::Experimental::Layout::GUTTER_OPTIONS) %>
      # @param system_arguments [Hash] <%= link_to_system_arguments_docs %>
